...
public class ApplicationService {
	...
	public String getApplicationSummary(String repositoryName) throws DataBaseOperationFailedException, ApplicationServiceException{
		...
		ImageRegistry imageRegistry=imageRegistryDAO.getImageRegistryByName(jsonObject.getString("imageRepository"));
	...
	@Path("/addService")
	@Consumes(MediaType.APPLICATION_JSON)
	public void addService(String applicationServiceData) throws DataBaseOperationFailedException, MarathonServiceException, InterruptedException, ApplicationServiceException{
		...
		try {
			...
			marathonService.postRequestToMarathon(addService);
			List<Subnet> subnetlist = networkDAO.getAllSubnet();
			String cidr="";			
			for(Subnet sub:subnetlist){
				cidr=sub.getCidr();				
			}
			logger.debug("----------CIDR  is  ---------"+cidr);
			logger.debug("----------Before  Network  script  called------------------------");			
			Thread.sleep(60000);
			String containerId=marathonService.getDockerContainerID();
			String newCider= PAASGenericHelper.getIncrementValue(cidr);
			ScriptService scriptService=new ScriptService();
			//scriptService.callScript(containerId, newCider);
			scriptService.executeFile(containerId, newCider);
